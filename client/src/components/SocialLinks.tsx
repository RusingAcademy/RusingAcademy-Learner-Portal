/**
 * SocialLinks — Shared social media links component
 * Used in footers across all portals for brand consistency.
 */
...
interface SocialLinksProps {
  ...
  className?: string;
  /** Variant: 'inline' for footer rows, 'stacked' for sidebar columns */
...
export default function SocialLinks({
  ...
  return (
    <div className={className ?? (variant === "inline" ? "flex items-center gap-3" : "flex flex-col gap-2")}>
      {SOCIAL_LINKS.map((s) => (
        <a
          key={s.label}
          href={s.href}
