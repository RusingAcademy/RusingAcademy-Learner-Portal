      ...
      <div className="max-w-[1200px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {reports.map((report, i) => (
            <div
              key={i}
              className="bg-white rounded-md border border-gray-200 p-5 hover:border-[#086FDD]/30 hover:shadow-sm transition-all cursor-pointer group"
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-lg bg-[#D4FBF7] flex items-center justify-center flex-shrink-0">
                  <span className="material-icons text-[#086FDD] text-[20px]">{report.icon}</span>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-gray-800 group-hover:text-[#086FDD] transition-colors">
                    {report.name}
                  </h3>
